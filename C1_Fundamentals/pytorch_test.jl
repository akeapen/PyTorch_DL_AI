...
# ╔═╡ 2f03f440-c635-11f0-bcae-6b131a118317
begin
	using Pkg; Pkg.instantiate()
	ENV["LD_LIBRARY_PATH"]="" # for CUDA
    # using Distributed
	using PlutoUI, Random, LinearAlgebra, SparseArrays, SignalAnalysis, StatsBase, SigMF, Parameters, JSON3, CSV, DataFrames, Interpolations, MLStyle, CUDA, CUDA.CUFFT, Dates, DSP, PyCall, LaTeXStrings, Memoize, LazyArrays, FFTW, FastRunningMedian, MLJClusteringInterface, MLJBase, Statistics , CairoMakie#, CondaPkg
    # Distributed.@everywhere using CairoMakie
	using ToeplitzMatrices
	using SpecialFunctions   # besselj
	using Distributions 		# Laplace
end

# ╔═╡ c3978e39-1d7c-415e-b6e8-d66fc9010907
html""" <style> main {
     margin: 0 auto;
     max-width: 1400px;
     padding-left: max(30px, 13%);
     padding-right: max(50px, 1%);
} </style> """

# ╔═╡ 3f8a0a3c-4d29-4ffe-9f59-191506094d3d
md""" 
_black_ $\;$ $(@bind is_black PlutoUI.CheckBox(default=true))
"""

# ╔═╡ 954354a4-5924-4577-9d57-ec46c98c4b11
if is_black
...
# ╔═╡ b10137a1-f4b7-42f8-941b-9be40ffd28bd
begin
	# Conda.pip_interop(true) # or specify a different environment if needed via , Conda.ROOTENV
	# Conda.pip("install", ["somepackage1", "somepackage2"])
	# Conda.pip("uninstall", ["somepackage1", "somepackage2"])
	
	PyCall.PYTHONHOME # "C:\\Users\\fe736f\\.julia\\conda\\3\\x86_64"
	Pkg.build("PyCall")
	# run(`$(PyCall.python) -m pip install sigmf`)
	# run(`$(PyCall.python) -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128`)
	# run(`$(PyCall.python) -m pip install scipy`) #Successfully installed scipy-1.16.3
	# run(`$(PyCall.python) -m pip install cupy-cuda12x`) #Successfully installed cupy-cuda12x-13.6.0 fastrlock-0.8.3
	# run(`$(PyCall.python) -m pip install cuda-toolkit`) # Successfully installed cuda-toolkit-13.0.1
end

# ╔═╡ 68aee2e9-298a-464e-8000-a9f8030a44c9
begin
	scipy_sig = pyimport("scipy.signal")
	cupyx_sig = pyimport("cupyx.scipy.signal")
	firls = scipy_sig.firls
	cupy  = pyimport("cupy")
	torch = pyimport("torch")
end

# ╔═╡ e364fc31-45ee-487c-8e53-e10aa63e11ac
# 3000-2796.05 == 203.95
...
SigMF = "dc1af1da-aa00-46a0-b3ad-5cde4383c966"
...
manifest_format = "2.0"
project_hash = "b1e02e0bfc04089a43928adc87c7e07478dac603"
...
[[deps.FastRunningMedian]]
deps = ["DataStructures"]
git-tree-sha1 = "b062354478434aa7d01df3c93ca6bdb315acf305"
uuid = "3a47ddbd-112b-4cc1-b00e-3746ec434ee5"
version = "0.3.1"
...
[[deps.SigMF]]
deps = ["Dates", "FileIO", "JSON3", "Mmap", "SHA", "StructTypes", "Tar"]
git-tree-sha1 = "92b7fd5f91717ac2dd71c4dc57365f48cc60b21f"
uuid = "dc1af1da-aa00-46a0-b3ad-5cde4383c966"
version = "0.2.0"
